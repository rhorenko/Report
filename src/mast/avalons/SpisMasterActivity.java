...
public class SpisMasterActivity extends Activity implements AdapterView.OnItemSelectedListener {
    ...
    private static final String[] mWandMContent = new String[] {
        ContactDbHelper._ID, ContactDbHelper.WORK, ContactDbHelper.MATERIAL,
        ContactDbHelper.VOLUME};
    ...
    private static final String[] mContentWandM = new String[] {
    	WandMDbHelper._ID, WandMDbHelper.WORK, WandMDbHelper.MATERIAL, 
    	WandMDbHelper.VOLUME};
    private int mYear;
    private int mMonth;
    private int mDay;
    static final int DATE_DIALOG_ID = 0;
    static final int DATE_DIALOG_ID1 = 1;
    public static final String EXT_WORK = "WORK";
	public static final String EXT_OBJECT = "OBJECT";
	public static final String EXT_COUNT = "COUNT";
	public static final String TAG = "SpisMasterActivity.class";
	@Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.spisan);
        Bundle extras = getIntent().getExtras();
        work   = extras.getString(EXT_WORK);
        object = extras.getString(EXT_OBJECT);
        count  = extras.getString(EXT_COUNT);
        Log.d(TAG, "work="+work);
        ...
        mWandMCursor = managedQuery(
                WandMProvider.CONTENT_URI, mWandMContent, null, null,null);//ORDER BY _ID ASC
        ...
        mWorkAdapter = new SimpleCursorAdapter(this, 
                ...
                new int[] {R.id.item}); 
        AddToTempMaster(work,object,count);
...
private void AddWorkDialog() {
    ...
    b.setTitle(getResources().getString(R.string.title_adding_work));
    b.setPositiveButton(
            R.string.btn_ok, new DialogInterface.OnClickListener() {
        public void onClick(DialogInterface dialog, int whichButton) {
            ContentValues values = new ContentValues(2);
...
void AddToTempMaster(String work,String object,String count) {
	Log.d("TAG", "AddToTempMaster");
    Button date=(Button)findViewById(R.id.date1);
    mWandMCursor = managedQuery(
            WandMProvider.CONTENT_URI, mWandMContent, "work='"+work+"'", null,null);//ORDER BY _ID ASC
   for (int i = 0; i < mWandMCursor.getCount(); i++) {
    	mWandMCursor.moveToPosition(i);
   ContentValues values = new ContentValues(2);
   values.put(TempDbHelper.OBJECT,    object);
   values.put(TempDbHelper.MATERIAL,  mWandMCursor.getString(2));
   values.put(TempDbHelper.VOLUME,    ""+Integer.parseInt(mWandMCursor.getString(3))*Integer.parseInt(count)); 
   values.put(TempDbHelper.DATE,      date.getText().toString()); 
   values.put(TempDbHelper.COMMENT,   "");
   values.put(TempDbHelper.FLAG, "-");
   values.put(TempDbHelper.FACTVOLUME,""+Integer.parseInt(mWandMCursor.getString(3))*Integer.parseInt(count) );
   values.put(TempDbHelper.FLAG_FACT, "+");
   values.put(TempDbHelper.WORK,""+   mWandMCursor.getString(1) );
